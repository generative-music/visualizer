...
const draw = svg(DIV_ID).size(DIAGONAL, DIAGONAL);

const stroke = el =>
  el.attr({ stroke: 'black', 'stroke-width': STROKE_WIDTH, 'fill-opacity': 0 });
...
window.save = (numImages = 1) => {
  clearInterval(running);

  for (let i = 1; i <= numImages; i += 1) {
    setTimeout(() => {
      makeNext(false, true);
      saveSvgAsPng.saveSvgAsPng(draw.node, `image${i}.png`, {
